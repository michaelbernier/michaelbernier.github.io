...
\subsection{Moindres carrés - estimation directe (intensité)}



\begin{frame}
    \frametitle{Estimation paramétrique directe par raffinement itératif}
...
\begin{frame}
    ...
    On peut résoudre cette équation par \textbf{régression linéaire} ou \textbf{recherche exhaustive}.
...
\subsection{Moindres carrés - estimation indirecte (mouvement affine)}

\begin{frame}
    \frametitle{Estimation paramétrique indirecte (modèle affine)}
	
    Supposons un mouvement plus complexe tel un modèle affine. 
    ...
    \right)
    \end{eqnarray*}
    et $\mathbf{p}=(a_0,a_1,a_2,b_0,b_1,b_2)$.\medskip
    
    L'expression de l'énergie à minimiser est:
    
    \begin{eqnarray}
    E(\mathbf{p}^{k}) = \displaystyle \sum_{(x,y)\in W} \big[\textbf{T}(\left(x,y\right);\mathbf{p}^{k}) - \textbf{d}(x,y)\big]^2 \label{EQ_INDIRECT_AFFINE}
    \end{eqnarray}
    
    On peut résoudre cette équation par \textbf{régression linéaire}.
...
\begin{frame}
    \frametitle{Estimation paramétrique indirecte (affine)}
    \framesubtitle{Algorithme de l'estimation paramétrique indirecte par régression linéaire}
    
    \begin{small}
        \begin{algorithm}[H]
            \Entree{\fbox{$I(t)$,$I(t+1)$}}
            \Sortie{\fbox{$\mathbf{p}$}: Paramètres du modèle de mouvement global de l'image}
            \medskip
            
            $\mathbf{d} \leftarrow$ Initialisation par bloc fixe ou H-S ou $\dots$ \;
            $\frac{\partial T}{\partial \textbf{p}} \leftarrow$ Calculer la matrice des dérivées partielles du modèle de transformation\;
            $\mathbf{p} \leftarrow \dfrac{\bigg[ \frac{\partial T}{\partial \textbf{p}} \bigg]^T \mathbf{d}}{\bigg[\frac{\partial T}{\partial \mathbf{p} } \bigg]
                \bigg[\frac{\partial T}{\partial \textbf{p}} \bigg]^T}$\; 
